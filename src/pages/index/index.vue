  ...
  <view class='index-page'>
    ...
    <view class='tab-wrapper'>
      <block v-for='(tab, index) in tabsList' :key='index'>
        <view class='tab' :class='active === index ? "active" : ""' @click='onChangeTab(index)'>{{ tab }}</view>
      </block>
    </view>
    <view class='item-wrapper'>
      <view class='item' v-for='(item,index) in [1,2,3]' :key='index'>
        <image class='item-icon' src='' />
        <text class='item-title'>腾讯视频</text>
        <button class='item-btn'>低至6元起</button>
      </view>
    </view>
  </view>
</template>
